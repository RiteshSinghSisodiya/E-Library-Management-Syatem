...
namespace WebApplication2
{
    public partial class adminBooksInventory : System.Web.UI.Page
    {
        ...
        void updateBookByID()
        {
            if (checkIfBookExists())
            {
                try
                {
                    ...
                    // Ensure the stock values are valid
                    if (global_actual_stock != actual_stock)
                    {
                        if (actual_stock < global_issued_books)
                        {
                            Response.Write("<script>alert('Actual Stock can't be less than the Issued Books');</script>");
                            return;
                        }
                        else
                        {
                    ...
                    string genres = string.Join(",", ListBox1.Items.Cast<ListItem>().Where(i => i.Selected).Select(i => i.Value));

                    string filepath = global_filepath; // Default to the existing image path
                    string filename = Path.GetFileName(FileUpload1.PostedFile.FileName);

                    // Check if a new file is uploaded
                    if (!string.IsNullOrEmpty(filename))
                    {
                        // Save the new file and update the filepath
        ...
        void getBookById()
        {
            try
            {
                ...
                if (dt.Rows.Count >= 1)
                {
                    ...
                    global_actual_stock = Convert.ToInt32(dt.Rows[0]["current_stock"]);
        ...
        void addNewBook()
        {
            ...
            {
                // Collect selected genres
                var selectedGenres = ListBox1.Items.Cast<ListItem>()
                                   .Where(i => i.Selected)
                                   .Select(i => i.Value);
